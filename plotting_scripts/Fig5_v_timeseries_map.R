...
ranks <- df %>% 
  ...
  rename(grand_total_articles_r = total_articles)
...
pct_select <- df %>%
  ...
  mutate(total_leading = sum(lead_match)) %>% 
  mutate(total_including = sum(matching) - sum(lead_match)) %>%
  #mutate(total_solo = sum(only_local)) %>% 
  select(`Publication Year`, v_name,facet_title, grand_total_articles, total_articles,total_including,total_leading) %>%
  unique() %>% 
  mutate(lead_pt = ((total_leading/total_articles)*100),
         #incl_nolead_pt = (((total_including + total_leading)/total_articles)*100)+0.2,
         incl_pt = (((total_including)/total_articles)*100)) %>% #,solo_pt = ((total_solo/total_articles)*100)+0.2
          
  mutate(no_involvement_pt = 100 -incl_pt -lead_pt) %>% 
  ...
  mutate(rm_lead = rollmean(lead_pt,k=3,align = 'right', fill = 1),
         ...
         rm_no_in = rollmean(no_involvement_pt,k=3,align = 'right',fill = 1)) %>% # ,rm_solo = rollmean(solo_pt,k=3,align = 'right',fill = 1)
  ...
  pivot_longer(cols=c('rm_lead', 'rm_incl','rm_no_in'), names_to = 'metric', values_to = 'rolling') %>% 
  filter(pub_year >= 1990 & pub_year < 2021) %>% 
  # add in volcano ranks (by n article) to break up plots into groups
  left_join(.,ranks, by = 'v_name') %>%
...
# create facet title with n articles
pct_select <- pct_select %>% 
  mutate(facet_title = paste(Original_name,'   n = ',grand_total_articles, sep = ""))



# Specific Plots
pct_select <- pct_select %>% 
  filter(Original_name == 'Eyjafjallajökull'|
           Original_name == 'Soufrière Hills'|
           Original_name == 'Pinatubo'|
           Original_name == 'Merapi')
...
filename <- sprintf('figs/supplement_figs/volc_timeseries_main.png')
...
gg <- ggplot(df_plot)+
  ...
  labs(y = 'Percentage \nof articles',
       x = 'Publication year')+
  xlim(c(1990,2021))+
  scale_color_manual(labels = c("Involved",
                                "Leading", 'Not included'),
                     values = c("#9dae11", "#4b5c09","#b730c6ff","red2"))

gg
  
ggsave(filename = filename, height = 45,width = 175, units = 'mm', scale = 1.1)
